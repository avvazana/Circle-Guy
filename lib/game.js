...
function createVariables(){
  ...
  danceAudio = new Audio('assets/audio/dancePacman.mp3');
...
function left() {
  //moving  div from left to right
  $("#pacman").clearQueue().stop().animate({
    left: "-=50"
  }, 1000, 'linear');
      ...
      if (delta > interval) {
        if(frameNumber === 0){
          ...
          if(framesScaredCountdown === -1) {
            ...
            if (noCoinsLeft()) {
              document.getElementById("status").innerHTML =
              "<div><img src='assets/winningPacman.gif' margin-right=\"20px\" width=\"100px\" height=\"100px\"><img src='assets/youWinGhosts.gif' width=\"100px\" height=\"100px\"></div>";
              frameNumber = 0;
            ...
            if (collidedGhosts.notScared.length) {
              collidedGhosts.notScared.forEach((ghost, idx) => {
                ghost.visible = false;

                let ex = '';
              });
              document.getElementById("status").innerHTML = "<img src='assets/youLose.gif' width=\"100px\" height=\"100px\"><br> Nice Try";
              ghostDifferences = Object.assign(pacmanDifferences);
            ...
            pacmanEl = document.getElementById('pacman');
            if(pacmanEl){
