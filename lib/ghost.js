...
class Ghost {
  ...
  getDifferencesAndUpdateFromTileToTarget(targetXCoord, targetYCoord, avoidant){
    ...
    if (ghostXCoord > targetXCoord){
      ...
      if (avoidant) {
        addToMoves(nonConstructiveMoves, nonConstructiveMoves, wallsNextToGhost, 'left', 'right');
      } else {
  ...
  updatePosition(){
    if (this.eaten){
      framesUntilStarting['red'] += frameNumber;
      framesUntilStarting['blue'] += frameNumber;
      framesUntilStarting['orange'] += frameNumber;
      framesUntilStarting['pink'] += frameNumber;
      this.eaten = false;
      this.scared = false;
      this.outOfHome = false;
...
class RedGhost extends Ghost {
  ...
  _updatePosition(){
    ...
    return this.getDifferencesAndUpdateFromTileToTarget(pacmanXCoord, pacmanYCoord, false);
  }
}
...
class PinkGhost extends Ghost {
  ...
  _updatePosition(){
    ...
    return this.getDifferencesAndUpdateFromTileToTarget(targetTile[0], targetTile[1], false);
  }
}
...
class OrangeGhost extends Ghost {
  ...
  _updatePosition(){
    let pacmanXCoord = pacman.xCoord;
    let pacmanYCoord = pacman.yCoord;
    return this.getDifferencesAndUpdateFromTileToTarget(pacmanXCoord, pacmanYCoord, true);
  }
}
