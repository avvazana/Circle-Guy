...
function updateGhosts(){
  ...
  return ghostDifferences.concat(redDifferences).concat(pinkDifferences).concat(blueDifferences).concat(orangeDifferences);
}
...
function updateOrangeGhost(){
  let ghost = ghosts[3];
  let randomMove = getRandomMove(ghost);
  let differences = differencesFromTileToTarget(ghost, 3, randomMove[0], randomMove[1]);
  return differences;
}
...
function differencesFromTileToTarget(ghost, ghostVal, targetXCoord, targetYCoord){
  ...
  if(ghost.eaten){
			ghost.eaten = false;
			ghost.scared = false;
			// ghost.out_of_home = false;
			return [[ghostYCoord, ghostXCoord],[16, 7]];
		}
